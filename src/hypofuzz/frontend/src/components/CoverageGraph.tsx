...
export function CoverageGraph({ reports }: Props) {
  ...
  const [axisOption, setAxisOption] = usePreference<string>(
    "graph_x_axis",
    "time",
  )
  ...
  useEffect(() => {
    ...
    const latests = Object.entries(reports).map(
      ([nodeid, points]) => points[points.length - 1],
    )
        ...
        Object.values(reports).forEach(points => {
          ...
          sortedPoints.forEach(point => {
            const distance = Math.sqrt(
              (x(xValue(point)) - mouseX) ** 2 +
                (y(point.branches) - mouseY) ** 2,
            )
